...
(function() {
 ...
 mraid.passbook = function(URL) {
 if (!URL) broadcastEvent(EVENTS.ERROR, 'PASSBOOK is required.', 'open');
 else bridge.executeNativeCall('passbook', 'url', URL);
 };
 
 mraid.tweet = function(text) {
 if (!text) broadcastEvent(EVENTS.ERROR, 'TWEET is required.', 'open');
 else bridge.executeNativeCall('tweet', 'text', text);
 };
